...
public class HouseDAO {
	...
	public long addnew(HouseDTO house) {
		...
		sb.append(
				"FloorIndex,TypeId,Direction,LookableDateTime,CheckInDateTime,OwnerName,OwnerPhoneNum,CreateDateTime,Description,IsDeleted,Latitude,Longitude)\n");
		...
		try {
			...
			long houseId = JDBCUtils.executeInsert(conn, sb.toString(), house.getRegionId(), house.getCommunityId(),
					...
					house.getOwnerName(), house.getOwnerPhoneNum(), house.getDescription(),house.getLatitude(),house.getLongitude());
	...
	public void update(HouseDTO house) {
		...
		try {
			...
			JDBCUtils.executeNonQuery(conn, sb.toString(), house.getRegionId(), house.getCommunityId(),
					...
					house.getArea(), house.getDecorateStatusId(), house.getTotalFloorCount(), house.getFloorIndex(),
					house.getTypeId(), house.getDirection(), house.getLookableDateTime(), house.getCheckInDateTime(),
					house.getOwnerName(), house.getOwnerPhoneNum(), house.getDescription(), house.getLatitude(),house.getLongitude(),house.getId()
					);
	...
	public HouseSearchResult search2(HouseSearchOptions options) {
		...
		if (!StringUtils.isEmpty(options.getKeywords())) {
			// namelike '%?%'
			sbSelect.append("or region.Name like ?\n");// 根据小区名字模糊搜索
			listParams.add("%" + options.getKeywords() + "%");
			
			sbSelect.append("or Status.Name like ?\n");// 根据小区名字模糊搜索
			listParams.add("%" + options.getKeywords() + "%");
			
			sbSelect.append("or  DecorateStatus.Name like ?\n");// 根据小区名字模糊搜索
			listParams.add("%" + options.getKeywords() + "%");
	...
	public boolean checkCollection(long userId, long houseId) {
		StringBuilder sb  = new StringBuilder();
		sb.append("select * from t_collections where userId = ? and houseId = ? and isDeleted = 0 \n");
		ResultSet rs =null;
		try {
			rs = JDBCUtils.executeQuery(sb.toString(), userId,houseId);
			if(rs.next()){
				return true;
			}
			return false;
	...
	public void addToCollection(long userId, long houseId) {
		StringBuilder sb  = new StringBuilder();
		sb.append("insert into t_collections(userId,houseId) values(?,?) \n");
		try {
			JDBCUtils.executeInsert(sb.toString(), userId,houseId);
		} catch (SQLException e) {
			throw new RuntimeException(e);
		}
	}
	
	public void cancelCollection(long userId,long houseId){
		StringBuilder sb  = new StringBuilder();
		sb.append("update t_collections set isDeleted = 1 where userId =? and houseId =? \n");
		try {
			JDBCUtils.executeNonQuery(sb.toString(), userId,houseId);
		} catch (SQLException e) {
			throw new RuntimeException(e);
		}
	}

	public HouseDTO[] recommented(long typeId,long cityId) {
		
		StringBuilder sb = new StringBuilder();
		ResultSet rs = null;
		List<HouseDTO> list = new ArrayList<>();
		sb.append(selectMainSQL);
		sb.append("where TypeId = ? and cityId = ? and h.isDeleted =0 \n");
		
		//找出该类型的所有的id，包装成数组，然后随机取出其中三个
		try {
			rs = JDBCUtils.executeQuery(sb.toString(), typeId,cityId);
			while(rs.next()){
				list.add(toDTO(rs));
			}
			int size = list.size();//数组的大小
			Set<Integer> randomSet =new HashSet<>(); 
			List<HouseDTO> recommList = new ArrayList<>();
			//
			//则把所有的list返回吧
			if(size<=3){
				return list.toArray(new HouseDTO[list.size()]);
			}else{
				while(randomSet.size()<3){	//大于3的时候要拿够3个才给走！
					
					int rand = new Random().nextInt(size);
					if(!randomSet.contains(rand)){//1 2 3 
						randomSet.add(rand);
					}
				}
				for(int id:randomSet){				
					recommList.add(list.get(id));
				}
				return recommList.toArray(new HouseDTO[recommList.size()]);
	...
	public HouseDTO[]  recommented(long cityId) {
		StringBuilder sb = new StringBuilder();
		ResultSet rs = null;
		List<HouseDTO> list = new ArrayList<>();
		sb.append(selectMainSQL);
		sb.append("where cityId = ? and h.isDeleted =0 \n");
