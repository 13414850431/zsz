    ...
    $(function(){
    	...
    	//刷新图形验证码
    	 ...
    	 $("#btnReg").click(function(){
    		 ...
    		 $.ajax({type:"post",url:"<%=ctxPath%>/User",
 				data:{action:"regSubmit",phoneNum:phoneNum,smsCode:smsCode,password:password,rePassword:rePassword},
 				success: function(result)
 				{
 					if(result.status=="ok")
					{
						alert("注册成功");
						window.location.href = "<%=ctxPath%>/User?action=login";
					}
 					else
					{
						alert(result.msg);
		...
		<div class="register clearfloat" id="main">
			<ul>
				...
				<li class="clearfloat">
					...
					<input type="password" id="rePassword" value="" class="shuru fl" placeholder="请再次输入密码" />
				</li>
			</ul>
