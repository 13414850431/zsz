...
public class HouseServlet extends BaseServlet {
	...
	public void view(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		long typeId=  house.getTypeId();
		long cityId = house.getCityId();
		HouseDTO[] recommHouses  = service.recommented(typeId,cityId);//拿到推荐房子
		
		/*
		//把推荐房子的第一张照片也拿到
		List<HousePicDTO> recommPicsList = new ArrayList<>();
		for(HouseDTO recomm:recommHouses){
			recommPicsList.add(service.getPics(recomm.getId())[0]);
		}
		
		
		req.setAttribute("recommPics", recommPicsList.toArray(new HousePicDTO[recommPicsList.size()]));
		*/
		...
		if(userId!=null){
			isCollection=houseService.checkCollection(userId, id);
			
		}
		
		req.setAttribute("isCollection", isCollection);
	...
	//把房子添加到收藏
	public void collecting(HttpServletRequest req,HttpServletResponse resp) throws IOException{
		
		//拿到用户的Id
		Long userId = FrontUtils.getCurrentUserId(req);
		
		long houseId = Long.parseLong(req.getParameter("houseId"));//拿到房子Id
		HouseService houseService = new HouseService();
		
		if(userId==null){
			writeJson(resp, new AjaxResult("error","收藏失败，请先登录"));
			return ;
		}
		
		
		if(!houseService.checkCollection(userId,houseId)){//如果没有收藏关系
			//那就添加到收藏咯
			houseService.addToCollection(userId,houseId);
			writeJson(resp, new AjaxResult("success","收藏成功"));
			return ;
			
		}else{
			
			//已经存在收藏关系,取消收藏
			houseService.cancelCollection(userId, houseId);
			writeJson(resp, new AjaxResult("cancle","已取消收藏"));
			return;
